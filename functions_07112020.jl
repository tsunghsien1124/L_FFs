function para_func(;
    ...
    e_size::Integer = 7,        # no. of endowment shock
    ...
    a_size_pos::Integer = 61,   # number of grid of positive asset holding for VFI
...
function find_threshold_func(
    ...
    #=
    # V_diff_itp = Spline1D(e_grid, V_diff, k = 1, bc = "extrapolate")
    V_diff_itp = Akima(e_grid, V_diff)
    if all(V_diff .> 0.0)
        e_p_lower = -Inf
        e_p_upper = e_grid[1]
        # e_p_thres = find_zero(e_p->V_diff_itp(e_p), (e_p_lower, e_p_upper), Bisection())
        e_p_thres = find_zero(e_p->V_diff_itp(e_p), e_p_upper, xatol = 1E-4)
    elseif all(V_diff .< 0.0)
        e_p_lower = e_grid[end]
        e_p_upper = Inf
        # e_p_thres = find_zero(e_p->V_diff_itp(e_p), (e_p_lower, e_p_upper), Bisection())
        e_p_thres = find_zero(e_p->V_diff_itp(e_p), e_p_lower, xatol = 1E-4)
    else
        e_p_lower = e_grid[findall(V_diff .<= 0.0)[end]]
        e_p_upper = e_grid[findall(V_diff .>= 0.0)[1]]
        e_p_thres = find_zero(e_p->V_diff_itp(e_p), (e_p_lower, e_p_upper), Bisection())
...
function price_func!(
    ...
    @unpack r_f, λ, a_size_neg, e_size, e_grid, e_ρ, e_σ, ν_p = parameters
    ...
    Threads.@threads for a_p_i in 1:a_size_neg
        ...
        for e_i in 1:e_size
            ...
            q_update[a_p_i,e_i] = repay_prob / ((1.0+r_f)*(1.0+λ))
        end
    end
