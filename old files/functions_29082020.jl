...
using ProgressMeter
using Parameters
using QuantEcon: rouwenhorst, tauchen, gridmake
using Plots
using Dierckx
using PrettyTables
using Roots
...
function para_func(;
    ...
    a_max::Real = 300,          # max of asset holding
    a_size_neg::Integer = 451,  # number of the grid of negative asset holding for VFI
    a_size_pos::Integer = 101,  # number of the grid of positive asset holding for VFI
...
function value_func!(
    ...
    if DSS_algo == 1
        ...
        for x_i in 1:x_size
            ...
            V_hat_bad = (ν*β_H)*(ψ_H*V_good_p[a_ind_zero:end,:].+(1-ψ_H)*V_bad_p)*x_Γ[x_i,:]
            V_hat_good = (ν*β_H)*V_good_p*x_Γ[x_i,:]

            # bad credit history
            for a_i in 1:a_size_pos
                a = a_grid_pos[a_i]
                V_bad_all = u_func.(e .+ (1+r_d)*a .- a_grid_pos, σ) + V_hat_bad
                V_bad_max = maximum(V_bad_all)
                V_bad_max_ind = findall(V_bad_all .== V_bad_max)[end]
                variables.V_bad[a_i,x_i] = V_bad_max
                variables.policy_a_bad[a_i,x_i] = a_grid_pos[V_bad_max_ind]
            end
            ...
            for a_i in 1:a_size
                a = a_grid[a_i]
                V_good_all = u_func.(e .+ (1+r_d*(a>0))*a .- q .* a_grid, σ) + V_hat_good
                V_good_max = maximum(V_good_all)
                V_good_max_ind = findall(V_good_all .== V_good_max)[end]
                variables.V_good_repay[a_i,x_i] = V_good_max
                if variables.V_good_default[a_i,x_i] > variables.V_good_repay[a_i,x_i]
                    ...
                    variables.policy_a_good[a_i,x_i] = a_grid[V_good_max_ind]
        ...
        for x_i in 1:x_size
            ...
            for a_i in 1:a_size_pos
                ...
                V_hat_bad_itp = Spline1D(a_grid_pos[a_bad_lower_ind:a_bad_upper_ind], V_hat_bad[a_bad_lower_ind:a_bad_upper_ind], k = 1, bc = "extrapolate")
                object_bad(a_p) = -(u_func(CoH - a_p, σ) + V_hat_bad_itp(a_p))
                res_bad = optimize(object_bad, a_grid_pos[a_bad_lower_ind], min(a_grid_pos[a_bad_upper_ind],CoH))
            ...
            for a_i in 1:a_size
                ...
                if V_good_max == -Inf
                    ...
                    variables.V_good_repay[a_i,x_i] = -Inf
                    variables.V_good[a_i,x_i] = variables.V_good_default[a_i,x_i]
                    variables.policy_a_good[a_i,x_i] = 0.0
                    variables.policy_d_good[a_i,x_i] = 1.0
                    ...
                    # solve it with interpolation method
                    V_hat_good_itp = Spline1D(a_grid[a_good_lower_ind:a_good_upper_ind], V_hat_good[a_good_lower_ind:a_good_upper_ind], k = 1, bc = "extrapolate")
                    qa_itp = Spline1D(a_grid[a_good_lower_ind:a_good_upper_ind], q[a_good_lower_ind:a_good_upper_ind].*a_grid[a_good_lower_ind:a_good_upper_ind], k = 1, bc = "extrapolate")
                    object_good(a_p) = -(u_func(CoH - qa_itp(a_p), σ) + V_hat_good_itp(a_p))
                    res_good = optimize(object_good, a_grid[a_good_lower_ind], min(a_grid[a_good_upper_ind],CoH))
