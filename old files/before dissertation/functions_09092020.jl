...
function para_func(;
    ...
    e_size::Integer = 2,        # no. of earnings shock
    ...
    a_size_pos::Integer = 81,   # number of the grid of positive asset holding for VFI
...
function value_func!(
    ...
    for x_i in 1:x_size
        ...
        V_hat_bad_itp = interpolate(a_grid_pos,V_hat_bad)
        V_hat_good = (ν*β)*V_good_p*x_Γ[x_i,:]
        V_hat_bad_itp = interpolate(a_grid,V_hat_good)
...
#=
plot(parameters.a_grid_neg, variables.q[1:parameters.a_ind_zero,:], seriestype=:scatter, legend=:bottomright)
...
α(λ) = (β*(1-ψ_B)*(1+r_d)) / ((1-λ)-β*ψ_B*(1+r_d))
Λ(λ) = β*(1-ψ_B+ψ_B*α(λ))
C(λ) = λ*θ / Λ(λ)
α_grid = α.(λ_grid)
Λ_grid = Λ.(λ_grid)
...
plot(λ_grid, C_grid, seriestype=:scatter)
=#
