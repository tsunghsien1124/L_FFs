...
function para_func(;
    ...
    # persistent shock
    e_grid, e_Γ = gausshermite(e_size);
    ...
    return (ψ_H = ψ_H, ψ_B = ψ_B, β_H = β_H, β_B = β_B, ξ = ξ, σ = σ, z = z,
            r_d = r_d, θ = θ, λ = λ, α = α, Λ = Λ, r_ld = r_ld, LR = LR, # ω = ω,
            # grid for asset holdings (VFI)
            ...
            # grid for asset holdings (density)
            ...
            # grid for idiosyncratic shocks
            e_ρ = e_ρ, e_σ = e_σ, e_size = e_size, e_grid = e_grid, e_Γ = e_Γ,
            ν_size = ν_size, ν_grid = ν_grid, ν_p = ν_p,
            x_size = x_size, x_grid = x_grid, x_ind = x_ind)
end
...
function var_func(parameters::NamedTuple)
    ...
    @unpack a_size, a_size_pos, a_ind_zero, a_grid = parameters
    ...
    q[1:(a_ind_zero-1),:] .= 1.0 / (1.0 + r_d + r_ld)
    ...
    # V_good = u_func.(repeat(exp.(e_grid)',a_size,ν_size) .+ repeat((1.0 .+ r_d*(a_grid .> 0.0)).*a_grid,1,x_size), σ)
    # V_bad = deepcopy(V_good[a_ind_zero:end,:])
...
function value_func!(
    ...
    @unpack a_size, a_size_pos, a_grid, a_grid_pos = parameters
    @unpack x_size, x_grid, x_ind, e_grid, e_Γ = parameters
    @unpack β_H, σ, ξ, ψ_H, ν_p, r_d, e_ρ, e_σ = parameters
    ...
    for x_i in 1:x_size
        ...
        e_i, ν_i = x_ind[x_i,:]
        q = q_p[:,e_i]
        qa_itp = Akima(a_grid, q.*a_grid)
        object_rbl(a_p) = qa_itp(a_p)
        res_rbl = optimize(object_rbl, a_grid[1], 0.0)
        a_rbl = Optim.minimizer(res_rbl)

        e_nodes = sqrt(2.0)*e_σ*e_grid .+ e_ρ*e

        # next-period value function of bad credit history
        V_bad_good_impatient = (1.0/sqrt(π)) * sum(interp2d(akima,a_grid,e_grid,V_good_p[:,:,1],a_grid_pos,e_nodes)*e_Γ, dims=2)
        V_bad_good_patient = (1.0/sqrt(π)) * sum(interp2d(akima,a_grid,e_grid,V_good_p[:,:,2],a_grid_pos,e_nodes)*e_Γ, dims=2)
        V_hat_bad_good = ν_p*V_bad_good_impatient + (1.0-ν_p)*V_bad_good_patient

        V_bad_bad_impatient = (1.0/sqrt(π)) * sum(interp2d(akima,a_grid_pos,e_grid,V_bad_p[:,:,1],a_grid_pos,e_nodes)*e_Γ, dims=2)
        V_bad_bad_patient = (1.0/sqrt(π)) * sum(interp2d(akima,a_grid_pos,e_grid,V_bad_p[:,:,2],a_grid_pos,e_nodes)*e_Γ, dims=2)
        V_hat_bad_bad = ν_p*V_bad_bad_impatient + (1.0-ν_p)*V_bad_bad_patient

        V_hat_bad = ψ_H*V_hat_bad_good + (1.0-ψ_H)*V_hat_bad_bad
        V_hat_bad_itp = Akima(a_grid_pos, V_hat_bad)

        # next-period value function of good credit history
        V_good_impatient = (1.0/sqrt(π)) * sum(interp2d(akima,a_grid,e_grid,V_good_p[:,:,1],a_grid,e_nodes)*e_Γ, dims=2)
        V_good_patient = (1.0/sqrt(π)) * sum(interp2d(akima,a_grid,e_grid,V_good_p[:,:,2],a_grid,e_nodes)*e_Γ, dims=2)

        V_hat_good = ν_p*V_good_impatient + (1.0-ν_p)*V_good_patient
        ...
        for a_i in 1:a_size_pos
            ...
            a_bad_lower_ind = max(1, V_bad_max_ind-1)
            a_bad_upper_ind = min(a_size_pos, V_bad_max_ind+1)

            # solve it with interpolation method
            object_bad(a_p) = -(u_func(CoH - a_p, σ) + (ν*β_H)*V_hat_bad_itp(a_p))
            res_bad = optimize(object_bad, a_grid_pos[a_bad_lower_ind], min(a_grid_pos[a_bad_upper_ind],CoH), GoldenSection())
        ...
        for a_i in 1:a_size
            a = a_grid[a_i]
            CoH = exp(e) + (1.0 + r_d*(a>0))*a

            # identify the optimal regions with discrete gridpoints
            V_good_all = u_func.(CoH .- q .* a_grid, σ) .+ V_hat_good
            V_good_max = maximum(V_good_all)

            if V_good_max == -Inf

                variables.V_good_repay[a_i,e_i,ν_i] = -Inf
                ...
                a_good_lower_ind = max(1, V_good_max_ind-1)
                a_good_upper_ind = min(a_size, V_good_max_ind+1)

                # solve it with interpolation method
                object_good(a_p) = -(u_func(CoH - qa_itp(a_p), σ) + V_hat_good_itp(a_p))
                res_good = optimize(object_good, max(a_grid[a_good_lower_ind],a_rbl), min(a_grid[a_good_upper_ind],CoH), GoldenSection())
...
function price_func!(
    ...
    @unpack x_size, x_grid, x_Γ = parameters
    ...
    for x_i in 1:x_size
        for a_p_i in 1:a_size_neg_nozero
            ...
            for x_p_i in 1:x_size
                e_p, ν_p = x_grid[x_p_i,:]
                if variables.policy_d_good[a_p_i,x_p_i] == 1.0
                    revenue += x_Γ[x_i,x_p_i] * ξ * e_p
                else
                    revenue += x_Γ[x_i,x_p_i] * (-a_grid_neg[a_p_i])
                end
            end
            q_update[a_p_i,x_i] = revenue / ((1+r_d+r_ld)*(-a_grid_neg[a_p_i]))
        end
    end
...
function price_func!(
    ...
    a_size_neg_nozero = length(a_grid_neg_nozero)
    q_update = ones(a_size_neg_nozero, x_size)
    x_update = zeros(a_size_neg_nozero, x_size)



    V_good_repay_avg = ν_Γ[1,1]*variables.V_good_repay[:,1:e_size] + ν_Γ[1,2]*variables.V_good_repay[:,(e_size+1):(2*e_size)]
    V_good_default_avg = ν_Γ[1,1]*variables.V_good_default[:,1:e_size] + ν_Γ[1,2]*variables.V_good_default[:,(e_size+1):(2*e_size)]
    V_good_diff = V_good_repay_avg - V_good_default_avg
    ...
    for x_i in 1:x_size
        ...
        for a_p_i in 1:a_size_neg_nozero
            println("$x_i, $a_p_i")
            if (V_good_diff[a_p_i,1] > 0) && (V_good_diff[a_p_i,end] > 0)
                x_update[a_p_i,x_i] = -Inf
                default_prob = cdf(dist, x_update[a_p_i,x_i])
                q_update[a_p_i,x_i] = (1-default_prob) / (1+r_d+r_ld)
            elseif (V_good_diff[a_p_i,1] < 0) && (V_good_diff[a_p_i,end] < 0)
                x_update[a_p_i,x_i] = Inf
                default_prob = cdf(dist, x_update[a_p_i,x_i])
                q_update[a_p_i,x_i] = (1-default_prob) / (1+r_d+r_ld)
            else
                ...
                x_threshold = find_zero(object_V_diff, e_grid[e_i])
                x_update[a_p_i,x_i] = x_threshold
                default_prob = cdf(dist, x_update[a_p_i,x_i])
                q_update[a_p_i,x_i] = (1-default_prob) / (1+r_d+r_ld)
            end
        end
...
function household_func!(
    ...
    if oneloop_algo == 1
        ...
        while crit > tol && iter < iter_max
            ...
            # check convergence
            crit_V_good = norm(variables.V_good .- V_good_p, Inf)
            ind_V_good = findall(abs.(variables.V_good .- V_good_p) .== crit_V_good)
            crit_V_bad = norm(variables.V_bad .- V_bad_p, Inf)
            ind_V_bad = findall(abs.(variables.V_bad .- V_bad_p) .== crit_V_bad)

            # println("G: $crit_V_good and B: $crit_V_bad")
            println("G: $crit_V_good at $ind_V_good and B: $crit_V_bad at $ind_V_bad")
...
function density_func!(
    ...
    while crit > tol && iter < iter_max
        ...
        for x_i in 1:x_size
            ...
            if any(variables.policy_d_good[:,x_i] .== 1.0)
...
data_spec = Any[#= 1=# "Number of Endowment"                parameters.e_size;
                ...
                #= 6=# "Maximum of Assets"                  parameters.a_grid[1];
                #= 7=# "Minimum of Assets"                  parameters.a_grid[end];
                ...
                #=14=# "Implied Leverage Ratio"             parameters.LR;
                ...
                #=18=# "Actual Leverage Ratio"              variables.aggregate_var[4]]

hl_LR = Highlighter(f      = (data,i,j) -> i == 14 || i == 18,
...
# λ_optimal = find_zero(solve_targeted, (λ_lower, λ_upper))
λ_optimal = find_zero(solve_targeted, (0.0994, 0.0995))
=#

#=
plot(parameters.a_grid_neg, variables.q[1:parameters.a_ind_zero,1:parameters.e_size], seriestype=:scatter, legend=:bottomright)
