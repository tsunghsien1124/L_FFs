...
Indicator_solve_equlibrium_given_λ_optimal = false
...
if Indicator_solve_equlibria_λ_min_and_max == true
    ...
    solve_economy_function!(variables_min, parameters; slow_updaing = slow_updating)

    variables_max = variables_function(parameters; λ = 1.0 - sqrt(parameters.ψ))
    solve_economy_function!(variables_max, parameters; slow_updaing = slow_updating)
    ...
    pretty_table(data_spec; header = ["Name", "λ minimum", "λ maximum"], alignment = [:c, :c, :c], formatters = ft_round(8), body_hlines = [3,7])
end

if Indicator_solve_equlibrium_given_λ_optimal == true
...
if Indicator_solve_stationary_equlibrium == true
    ...
    ζ_d_search = 0.2200
    ν_p_search = 0.1300 # collect(0.01002:0.00002:0.01008)
