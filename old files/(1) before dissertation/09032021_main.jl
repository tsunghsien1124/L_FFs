...
function para_func_fixed_wage(;
    ...
    K_fixed_wage = exp(e_SS)*(α/(i+δ))^(1.0/(1.0-α))
    w = (1.0-α)*(K_fixed_wage^α)*(exp(e_SS)^(-α))
...
function aggregate_func!(
    ...
    # asset-to-debt ratio (or capital-loan-to-deposit ratio)
    ...
    for x_i in 1:x_size
        ...
        for a_μ_i in 1:a_size_μ
            ...
            a_p = clamp(policy_a_itp(a_μ), a_grid[1], a_grid[end])
            if a_p < 0.0
                 num_avg_i += variables.μ[a_μ_i,e_i,ν_i] * (1.0-policy_d_itp(a_μ)) * (1/q_itp(a_p)-1.0) * 100
                 den_avg_i += variables.μ[a_μ_i,e_i,ν_i] * (1.0-policy_d_itp(a_μ))
            end
        end
    end

    variables.aggregate_var[8] = num_avg_i/den_avg_i
    ...
    for x_i in 1:x_size
        ...
        for a_μ_i in 1:a_size_μ
            ...
            a_p = clamp(policy_a_itp(a_μ), a_grid[1], a_grid[end])
            if a_p < 0.0
                num_avg_i += (a_p/qa_itp(a_p)-1.0) * 100
                den_avg_i += 1
            end
        end
    end

    variables.aggregate_var[9] = num_avg_i/den_avg_i
end
...
function results_η_func(;
    ...
    for η_i in 1:η_size
        if η_i == 1
            ...
            parameters_NFF, variables_NFF, parameters_FF, variables_FF = λ_optimal_func(η_grid[η_i], a_min, a_size_neg, λ_min_adhoc = results_A_FF[η_i-1,3], e_ρ, e_σ)
        end
    ...
    header = ["Garnishment Rate", "Rental Rate (%)", "Multiplier", "Liquidity Premium (%)",
              ...
              "Percentage of Defaulters", "Percentage in Debt", "Debt-to-Earning Ratio", "Average Loan Rate (%)"]

    return results_A_NFF, results_V_NFF, results_μ_NFF, results_A_FF, results_V_FF, results_μ_FF, symbol, header
end

#=
results_A_NFF, results_V_NFF, results_μ_NFF, results_A_FF, results_V_FF, results_μ_FF, symbol, header = results_η_func(η_min = 0.10, η_max = 0.20, η_step = 0.05, a_min = -3.50, e_ρ = 0.95, e_σ = 0.10)
...
results_A_NFF_temp = results_A_NFF
results_A_FF_temp = results_A_FF
...
results_A_NFF = vcat(results_A_NFF, results_A_NFF_temp[2:end,:])
results_A_FF = vcat(results_A_FF, results_A_FF_temp[2:end,:])
@save "results_eta_0.10_0.80_0.05.bson" results_A_NFF results_A_FF symbol header
=#

@load "results_eta_0.10_0.80_0.05.bson" results_A_NFF results_A_FF symbol header

function results_func(;
    results_A_NFF::Array{Float64,2},
    results_A_FF::Array{Float64,2},
    a_min::Real,
    e_ρ::Real,
    e_σ::Real
    )
    ...
    # initialize results matrices
    results_A_NFF_new = zeros(η_size,14)
    results_V_NFF = zeros(parameters.a_size, parameters.e_size, parameters.ν_size, η_size)
    results_q_NFF = zeros(parameters.a_size_neg, parameters.e_size, η_size)
    results_μ_NFF = zeros(parameters.a_size_μ, parameters.e_size, parameters.ν_size, η_size)

    results_A_FF_new = zeros(η_size,14)
    results_V_FF = zeros(parameters.a_size, parameters.e_size, parameters.ν_size, η_size)
    results_q_FF = zeros(parameters.a_size_neg, parameters.e_size, η_size)
    results_μ_FF = zeros(parameters.a_size_μ, parameters.e_size, parameters.ν_size, η_size)

    # compute the optimal multipliers with different η
    for η_i in 1:η_size
        parameters_NFF = para_func(η = η_grid[η_i], a_min = a_min, a_size_neg = a_size_neg, λ = results_A_NFF[η_i,3], e_ρ = e_ρ, e_σ = e_σ)
        variables_NFF = var_func(parameters_NFF, load_initial_values = 0)
        solve_func!(variables_NFF, parameters_NFF)

        parameters_FF = para_func(η = η_grid[η_i], a_min = a_min, a_size_neg = a_size_neg, λ = results_A_FF[η_i,3], e_ρ = e_ρ, e_σ = e_σ)
        variables_FF = var_func(parameters_FF, load_initial_values = 0)
        solve_func!(variables_FF, parameters_FF)

        # record results
        results_A_NFF_new[η_i,1] = parameters_NFF.η
        results_A_NFF_new[η_i,2] = parameters_NFF.r_k*100
        results_A_NFF_new[η_i,3] = parameters_NFF.λ
        results_A_NFF_new[η_i,4] = parameters_NFF.r_lp*100
        results_A_NFF_new[η_i,5] = parameters_NFF.K
        results_A_NFF_new[η_i,6:end] .= variables_NFF.aggregate_var
        results_V_NFF[:,:,:,η_i] = variables_NFF.V
        results_q_NFF[:,:,η_i] = variables_NFF.q
        results_μ_NFF[:,:,:,η_i] = variables_NFF.μ

        results_A_FF_new[η_i,1] = parameters_FF.η
        results_A_FF_new[η_i,2] = parameters_FF.r_k*100
        results_A_FF_new[η_i,3] = parameters_FF.λ
        results_A_FF_new[η_i,4] = parameters_FF.r_lp*100
        results_A_FF_new[η_i,5] = parameters_FF.K
        results_A_FF_new[η_i,6:end] .= variables_FF.aggregate_var
        results_V_FF[:,:,:,η_i] = variables_FF.V
        results_q_FF[:,:,η_i] = variables_FF.q
        results_μ_FF[:,:,:,η_i] = variables_FF.μ
    end
    ...
    return results_A_NFF_new, results_V_NFF, results_q_NFF, results_μ_NFF, results_A_FF_new, results_V_FF, results_q_FF, results_μ_FF, symbol, header
end

results_A_NFF_new, results_V_NFF, results_q_NFF, results_μ_NFF, results_A_FF_new, results_V_FF, results_q_FF, results_μ_FF, symbol, header = results_func(results_A_NFF = results_A_NFF,
               results_A_FF = results_A_FF,
               a_min = -3.50,
               e_ρ = 0.95,
               e_σ = 0.10)

@save "results_10032021.bson" results_A_NFF_new results_V_NFF results_q_NFF results_μ_NFF results_A_FF_new results_V_FF results_q_FF results_μ_FF symbol header
@load "results_10032021.bson" results_A_NFF_new results_V_NFF results_q_NFF results_μ_NFF results_A_FF_new results_V_FF results_q_FF results_μ_FF symbol header

# liquidity premium and leverage ratio
header[9] = "Leverage Ratio"
results_A_NFF_new[:,9] = (results_A_NFF_new[:,9].*results_A_NFF_new[:,7])./results_A_NFF_new[:,8]
results_A_FF_new[:,9] = (results_A_FF_new[:,9].*results_A_FF_new[:,7])./results_A_FF_new[:,8]
...
plot_all = plot(layout = (plot_row,plot_col),
                size=(plot_col*600,plot_row*350),
                box = :on,
...
plot_all
savefig(plot_all, "plot_LP_LR.pdf")

# liquidity premium and physical capital
header[5] = "Physical Capital"
...
plot_all
savefig(plot_all, "plot_LP_K.pdf")

# two channels
η_grid = results_A_FF_new[:,1]
η_size = length(η_grid)
a_min = -3.50
e_ρ = 0.95
e_σ = 0.10
...
for η_i in 1:η_size
    parameters_FF = para_func_fixed_wage(η = η_grid[η_i], a_min = a_min, a_size_neg = a_size_neg, λ = results_A_FF[η_i,3], e_ρ = e_ρ, e_σ = e_σ)
    variables_FF = var_func(parameters_FF, load_initial_values = 0)
    solve_func!(variables_FF, parameters_FF)
    deposits_fixed_wage[η_i] = variables_FF.aggregate_var[2]
end
...
# aggregate variables
# results_A_NFF_new = hcat(results_A_NFF_new[:,1:12],results_A_NFF_new[:,14])
# results_A_FF_new = hcat(results_A_FF_new[:,1:12],results_A_FF_new[:,14])
header[10] = "Percentage of Filers (%)"
header[11] = "Percentage in debt (%)"

plot_row = 2
plot_col = 2
plot_size = plot_row * plot_col
plot_ordering = [10,11,12,13]
# plot_ordering = [4,5,6,7,9,10,11,12,13]
plot_title = [header[i] for i in plot_ordering]
plot_all = plot(layout = (plot_row,plot_col),
                size=(plot_col*600,plot_row*350),
...
for sp_i in 1:plot_size
    ...
    plot_all = plot!(subplot = sp_i,
                     ...
                     margin = 4mm)
    if sp_i == plot_size
        η_temp = vcat(results_A_FF_new[1:10,1],results_A_FF_new[12:end,1])
        i_temp = vcat(results_A_FF_new[1:10,plot_index],results_A_FF_new[12:end,plot_index])
        plot_all = plot!(subplot = sp_i,
                         η_temp,
                         i_temp,
                         xticks = 0.1:0.1:0.8,
...
plot_all
savefig(plot_all, "plot_aggregate_vars.pdf")
...
function results_CEV_func(
    ...
    for η_i in 1:η_size, x_i in 1:parameters.x_size
        ...
        V_itp_old = Akima(parameters.a_grid, results_V[:,e_i,ν_i,end])
...
for η_i in 1:η_size
    CEV_comparison_NFF[η_i] = sum(results_CEV_NFF[:,:,:,η_i] .* results_μ_NFF[:,:,:,η_i])
    CEV_comparison_FF[η_i] = sum(results_CEV_FF[:,:,:,η_i] .* results_μ_FF[:,:,:,η_i])
end

plot_col = 1
plot_row = 1

plot_all = plot(size=(plot_col*600,plot_row*500),
                box = :on,
                xtickfont = font(12, "Computer Modern", :black),
                ytickfont = font(12, "Computer Modern", :black),
                titlefont = font(18, "Computer Modern", :black),
                guidefont = font(16, "Computer Modern", :black),
                legendfont = font(12, "Computer Modern", :black))
plot_all = plot!(results_A_NFF_new[:,1],
                 CEV_comparison_NFF,
                 xticks = 0.1:0.1:0.8,
                 seriestype = :path,
                 markershapes = :auto,
...
plot_all = plot!(results_A_NFF_new[:,1],
                 ...
                 title = "Welfare (CEV)",
                 margin = 4mm)
plot_all = vline!([η_grid[argmax(CEV_comparison_NFF)]],
                  color = 1,
                  lw = 3,
                  ls = :dot,
                  label = "")
plot_all = vline!([η_grid[argmax(CEV_comparison_FF)]],
                  color = 2,
                  lw = 3,
                  ls = :dot,
                  label = "")
plot_all
savefig(plot_all, "plot_CEV.pdf")
