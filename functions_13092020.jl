...
function para_func(;
    ...
    ν_p::Real = 0.20,           # probability of patience
    a_min::Real = -4.5,         # min of asset holding
    a_max::Real = 300.0,        # max of asset holding
    a_size_neg::Integer = 901,  # number of the grid of negative asset holding for VFI
    a_size_pos::Integer = 301,  # number of the grid of positive asset holding for VFI
...
function var_func(parameters::NamedTuple)
    ...
    # return outputs
    variables = mut_var(q, V_good, V_good_repay, V_good_default, V_bad, policy_a_good, policy_a_bad, policy_d_good, μ_good, μ_bad, μ_Γ, aggregate_var)
    return variables
end
...
function value_func!(
    ...
    Threads.@threads for x_i in 1:x_size
        ...
        q = q_p[:,e_i,ν_i]
        q_itp = Akima(a_grid, q)
        qa = q .* a_grid
        qa_min = minimum(qa)
        qa_min_ind = findall(qa .== qa_min)[]
        ...
        function gradient_rbl!(G, a_p)
            G[1] = derivative(q_itp, a_p[1])*a_p[1] + q_itp(a_p[1])
        end
        if a_grid[qa_min_ind] <= a_grid[1]
            initial_rbl = a_grid[1] + 10^(-5)
        elseif a_grid[qa_min_ind] >= 0.0
            initial_rbl = 0.0 - 10^(-5)
        else
            initial_rbl = a_grid[qa_min_ind]
        end
        res_rbl = optimize(object_rbl, gradient_rbl!, [a_grid[1]], [0.0], [initial_rbl], Fminbox(GradientDescent()))
        ...
        for a_i in 1:a_size
            ...
            # println("x_i = $x_i and a_i = $a_i")
            # println("CoH = $CoH")
            # println("qa_rbl = $(qa_itp(a_rbl))")

            #=
            if (CoH - qa_itp(a_rbl) <= 0.0) # || (CoH - a_rbl <= 0.0)

                variables.V_good_repay[a_i,e_i,ν_i] = u_func(CoH .- qa_itp(a_rbl), σ)
            ...
            =#
                ...
                #=
                    ...
                    if a_grid[V_good_max_ind] >= CoH
                        initial_good = CoH - 10^(-5)
                    elseif a_grid[V_good_max_ind] <= a_rbl
                        initial_good = a_rbl + 10^(-5)
                    else
                        initial_good = a_grid[V_good_max_ind]
                    end
                    res_good = optimize(object_good, gradient_good!, [a_rbl], [CoH], [initial_good], Fminbox(GradientDescent()))
...
function get_μ_ind(
    ...
    return convert(Integer, (ν_p_i-1)*(a_size_μ_*e_size_) + (e_p_i-1)*(a_size_μ_) + a_p_i)
end
...
function density_func!(
    ...
    @unpack ψ_H = parameters
    @unpack x_size, x_ind, e_Γ, e_size, ν_Γ, ν_size = parameters
    @unpack a_size, a_grid, a_size_pos, a_grid_pos = parameters
    @unpack a_size_μ, a_grid_μ, a_size_pos_μ, a_grid_pos_μ, a_ind_zero_μ = parameters
    @unpack μ_good_ind, μ_bad_ind = parameters

    μ_size_good = a_size_μ * x_size
    μ_size_bad = a_size_pos_μ * x_size

    #  Threads.@threads
    @showprogress 1 "Iterating good credit history: " for μ_good_i in 1:μ_size_good

        a_i, e_i, ν_i = μ_good_ind[μ_good_i,:]
        a_μ = a_grid_μ[a_i]
        policy_a_good_itp = Akima(a_grid, variables.policy_a_good[:,e_i,ν_i])
        ...
        if abs(a_μ - a_repay) <= abs(a_μ - a_default)
            ...
            a_p = clamp(policy_a_good_itp(a_μ), a_grid[1], a_grid[end])
            ind_lower_a_p = findall(a_grid_μ .<= a_p)[end]
            ind_upper_a_p = findall(a_p .<= a_grid_μ)[1]
            ...
            for x_p_i in 1:x_size
                e_p_i, ν_p_i = x_ind[x_p_i,:]
                μ_good_p_i_lower = get_μ_ind(ind_lower_a_p, e_p_i, ν_p_i; a_size_μ_ = a_size_μ, e_size_ = e_size)
                μ_good_p_i_upper = get_μ_ind(ind_upper_a_p, e_p_i, ν_p_i; a_size_μ_ = a_size_μ, e_size_ = e_size)
                variables.μ_Γ[μ_good_i,μ_good_p_i_lower] = e_Γ[e_i,e_p_i] * ν_Γ[ν_i,ν_p_i] * weight_lower
                variables.μ_Γ[μ_good_i,μ_good_p_i_upper] = e_Γ[e_i,e_p_i] * ν_Γ[ν_i,ν_p_i] * weight_upper
            end
        ...
        # become bad
            ...
            for x_p_i in 1:x_size
                e_p_i, ν_p_i = x_ind[x_p_i,:]
                μ_bad_p_i = μ_size_good + get_μ_ind(1, e_p_i, ν_p_i; a_size_μ_ = a_size_pos_μ, e_size_ = e_size)
                variables.μ_Γ[μ_good_i,μ_bad_p_i] = e_Γ[e_i,e_p_i] * ν_Γ[ν_i,ν_p_i]
            end
        end

        dropzeros!(variables.μ_Γ)

    end

    # @showprogress 2 "Iterating bad credit history: "
    Threads.@threads for μ_bad_i in 1:μ_size_bad

        a_i, e_i, ν_i = μ_bad_ind[μ_bad_i,:]
        a_μ = a_grid_pos_μ[a_i]
        policy_a_bad_itp = Akima(a_grid_pos, variables.policy_a_bad[:,e_i,ν_i])

        a_p = clamp(policy_a_bad_itp(a_μ), a_grid_pos[1], a_grid_pos[end])
        ind_lower_a_p = findall(a_grid_pos_μ .<= a_p)[end]
        ind_upper_a_p = findall(a_p .<= a_grid_pos_μ)[1]
        ...
        for x_p_i in 1:x_size
            ...
            # become good
            μ_good_p_i_lower = get_μ_ind(a_ind_zero_μ-1+ind_lower_a_p, e_p_i, ν_p_i; a_size_μ_ = a_size_μ, e_size_ = e_size)
            μ_good_p_i_upper = get_μ_ind(a_ind_zero_μ-1+ind_upper_a_p, e_p_i, ν_p_i; a_size_μ_ = a_size_μ, e_size_ = e_size)
            variables.μ_Γ[μ_size_good+μ_bad_i,μ_good_p_i_lower] = (1.0-ψ_H) * e_Γ[e_i,e_p_i] * ν_Γ[ν_i,ν_p_i] * weight_lower
            variables.μ_Γ[μ_size_good+μ_bad_i,μ_good_p_i_upper] = (1.0-ψ_H) * e_Γ[e_i,e_p_i] * ν_Γ[ν_i,ν_p_i] * weight_upper

            # remain bad
            μ_bad_p_i_lower = μ_size_good + get_μ_ind(ind_lower_a_p, e_p_i, ν_p_i; a_size_μ_ = a_size_pos_μ, e_size_ = e_size)
            μ_bad_p_i_upper = μ_size_good + get_μ_ind(ind_upper_a_p, e_p_i, ν_p_i; a_size_μ_ = a_size_pos_μ, e_size_ = e_size)
            variables.μ_Γ[μ_size_good+μ_bad_i,μ_bad_p_i_lower] = ψ_H * e_Γ[e_i,e_p_i] * ν_Γ[ν_i,ν_p_i] * weight_lower
            variables.μ_Γ[μ_size_good+μ_bad_i,μ_bad_p_i_upper] = ψ_H * e_Γ[e_i,e_p_i] * ν_Γ[ν_i,ν_p_i] * weight_upper
        end
    end

    variables.μ_Γ = variables.μ_Γ ./ sum(variables.μ_Γ, dims=2)
    MC = MarkovChain(variables.μ_Γ)
    SD = stationary_distributions(MC)

    variables.μ_good = reshape(SD[][1:μ_size_good],a_size_μ,e_size,ν_size)
    variables.μ_bad = reshape(SD[][(μ_size_good+1):end],a_size_pos_μ,e_size,ν_size)
end
=#
...
data_spec = Any[#= 1=# "Number of Endowment"                parameters.e_size;
                ...
                #=18=# "Leverage Ratio (Demand)"            variables.aggregate_var[4]]
